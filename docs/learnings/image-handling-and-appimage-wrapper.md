...
2. **One fallback to rule them all** — swap to `/placeholder.png` on `onError`, implemented once
3. **One file to update** — if `next/image` changes its API or we want to add a loading skeleton, it happens in one place
...
- `components/ui/AppImage.tsx` — created in issue #7
